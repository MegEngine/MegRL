...
from baserl.data.env import BaseVectorEnv, DummyVectorEnv

class CCCiter(object):
    def __init__(self):
        self.n = 10
    ...
    def collect(
        ...
        while True:
            ...
            if random:
                ...
                assert isinstance(result.act, np.ndarray)
                act = result.act
                if self.exploration_noise:
                    act = self.policy.exploration_noise(act, self.data)
                log_actions.append(act)
            ...
            if np.any(done):
                ...
                obs_reset, info = self.env.reset(env_ind_global)
                if self.preprocess_fn:
