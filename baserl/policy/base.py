...
class BasePolicy(ABC, M.Module):
    ...
    * :meth:`~tianshou.policy.BasePolicy.update`: the main interface for training, \
    ...
    def soft_update(self, tgt, src, tau: float) -> None:
        """Softly update the parameters of target module towards the parameters \
        of source module."""
        for tgt_param, src_param in zip(tgt.parameters(), src.parameters()):
            tgt_param._reset(tau * src_param.numpy() + (1 - tau) * tgt_param.numpy())
    ...
    def map_action(self, act: Union[Batch, np.ndarray]) -> Union[Batch, np.ndarray]:
        ...
        if isinstance(self.action_space, gym.spaces.Box) and \
            ...
            if self.action_type == "continuous":
                print(type(self.action_space), self.action_space.low, self.action_space.high)
                print(type(act))
                assert 0
        return act
    ...
    def update(self, sample_size: int, buffer: Optional[ReplayBuffer],     
            update_fn,**kwargs: Any) -> Dict[str, Any]:
    ...
    def compute_episodic_return(
        ...
        if v_s_ is None:
            ...
            assert isinstance(v_s_, np.ndarray)
    ...
    def compute_nstep_return(
        ...
        target_q_mge = target_q_fn(buffer, terminal)  # (bsz, ?)
        if isinstance(target_q_mge, mge.Tensor):
            target_q_mge = target_q_mge.numpy()
        assert isinstance(target_q_mge, np.ndarray)
        target_q = target_q_mge.reshape(bsz, -1)
        ...
        batch.returns = mge.Tensor(target_q)
        if hasattr(batch, "weight"):  # prio buffer update
            batch.weight = mge.Tensor(batch.weight)
        return batch
