...
def _create_value(
    ...
    :param bool stack: whether to stack or to concatenate. E.g. if inst has shape of
        ...
        otherwise (10, 5)
    """
    has_shape = isinstance(inst, np.ndarray)
...
def _parse_value(obj: Any) -> Optional[Union["Batch", np.ndarray]]:
    if isinstance(obj, Batch):  # most often case
        return obj
    elif (isinstance(obj, np.ndarray) and
          issubclass(obj.dtype.type, (np.bool_, np.number))) or obj is None:  # third often case
        return obj
...
class Batch:
    ...
    def stack_(self, batches: Sequence[Union[dict, "Batch"]], axis: int = 0) -> None:
        ...
        for shared_key, value in zip(keys_shared, values_shared):
            # second often
            if all(isinstance(element, (Batch, dict)) for element in value):
    ...
    def update(
        ...
        if batch is None:
            self.update(kwargs) # kwargs serves as "batch"
            return
