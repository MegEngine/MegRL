    ...
    def load_func(k, v):
        ...
        if value_to_load.shape != v.shape:
            if value_to_load.size == v.size:  # shape mismatched but size the same
                logger.warning(
                    "Shape mismatched but size matched, will reshape and load. Please check."
                )
                logger.warning(f"{k}{v.shape} load from {load_key}{value_to_load.shape}")
                return value_to_load.reshape(v.shape)
            if not strict:
                logger.warning("Shape mismatched keys, will skip loadding. Please check.")
                logger.warning(f"{k}{v.shape} load from {load_key}{value_to_load.shape}")
                return v.numpy()
            else:
